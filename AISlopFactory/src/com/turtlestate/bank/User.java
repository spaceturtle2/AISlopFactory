...
/**
 * Shared User model used by BankingApp and StockSimulator.
 * Keep serialVersionUID stable across both apps.
 */
public class User implements Serializable {
    ...
    public int getPortfolioQuantity(String ticker) {
        return getPortfolio().getOrDefault(ticker, 0);
    }
    ...
    public synchronized boolean withdraw(double amount) {
        if (amount > 0 && Double.isFinite(amount)) {
            this.balance -= amount; // allows negative (debt)
            return true;
        }
        return false;
    }
